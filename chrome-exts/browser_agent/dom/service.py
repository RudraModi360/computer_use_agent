...
from browser_agent.dom.enhanced_snapshot import (
	REQUIRED_COMPUTED_STYLES,
	build_snapshot_lookup,
)
from browser_agent.dom.serializer.clickable_elements import ClickableElementDetector
...
class DomService:
	...
	async def _get_all_trees(self, target_id: TargetID, detect_js_listeners: bool = False) -> TargetAllTrees:
		cdp_session = await self.browser_session.get_or_create_cdp_session(target_id=target_id, focus=False)
	...
	async def get_dom_tree(
		...
		iframe_depth: int = 0,
		detect_js_listeners: bool = False,
		...
		trees = await self._get_all_trees(target_id, detect_js_listeners=detect_js_listeners)
