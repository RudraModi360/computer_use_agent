...
class LMMEngineLlamaCpp(LMMEngine):
    """Engine for llama.cpp server using OpenAI-compatible API.
    
    llama.cpp server provides an OpenAI-compatible API when run with:
    ./server -m model.gguf --port 8080
    
    Or for vision models:
    ./server -m model.gguf --port 8080 --mmproj mmproj.gguf
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # model parameter is optional for llama.cpp as it serves one model
    ...
    def generate(self, messages, temperature=0.0, max_new_tokens=None, **kwargs):
        # llama.cpp server doesn't require API key by default
        api_key = self.api_key or os.getenv("LLAMACPP_API_KEY") or "not-needed"
        
        # Default llama.cpp server endpoint
        base_url = self.base_url or os.getenv("LLAMACPP_BASE_URL") or "http://localhost:8080/v1"
