...
class SatelliteShell:
    ...
    def execute(self, command: str, timeout: int = config.SHELL_TIMEOUT) -> str:
        if self.is_busy:
                ...
                for attempt in range(3):
                    try:
                        ...
                        if attempt < 2:
                            time.sleep(1)
                            continue
                        return "Error: Connection refused. Shell might be closed."
...
class ShellManager:
    ...
    def get_available_shell(self) -> SatelliteShell:
        ...
        # 3. Spawn new satellite
        # Configurable Visibility
        print(f"[ShellManager] Spawning new Shell Satellite (UI={'ON' if config.SHELL_SHOW_UI else 'OFF'})...")
        
        flags = subprocess.CREATE_NEW_CONSOLE if config.SHELL_SHOW_UI else subprocess.CREATE_NO_WINDOW
